...
def upload_players_to_gcs(destination_bucket):
    ...
    run_gcs_task(destination_bucket, PLAYERS_TABLE, 'atp_players.parquet', 'downloads/players/atp_players_transformed.parquet')
...
def upload_matches_to_gcs(destination_bucket, MATCH_YEARS):
    ...
    for match in MATCH_YEARS:
        run_gcs_task(destination_bucket, MATCHES_TABLE, f'atp_matches_{match}.parquet', f'downloads/matches/atp_matches_{match}_transformed.parquet')
...
def upload_rankings_to_gcs(destination_bucket, RANKING_YEARS):
    ...
    for rank in RANKING_YEARS:
        run_gcs_task(destination_bucket, RANKINGS_TABLE, f'atp_rankings_{rank}.parquet', f'downloads/rankings/atp_rankings_{rank}_transformed.parquet')
